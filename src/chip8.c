...
// table of bitmasks and values used to match CHIP-8 opcodes
static const struct {
    uint16_t mask;
    uint16_t value;
} INSTRUCTION_MASKS[] = {
...
int
chip8_instruction_decode(struct instruction* inst, uint16_t code)
{
...
const char*
chip8_instruction_name(const struct instruction* inst)
{
...
// common function interface for CHIP-8 operations
...
int
chip8_operation_apply(struct chip8* chip8, const struct instruction* inst)
{
    assert(chip8 != NULL);
    assert(inst != NULL);
...
chip8_step(struct chip8* chip8)
{
    ...
    int rc = chip8_instruction_decode(&inst, code);
    if (rc != CHIP8_OK) {
        return rc;
    }

    rc = chip8_operation_apply(chip8, &inst);
    if (rc != CHIP8_OK) {
        return rc;
    }
